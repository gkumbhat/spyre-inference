...
- **dtype**: float16, plus bfloat16 for checkpoints that overflow it (`TorchSpyrePlatform._default_dtype` picks bf16 for a gemma-4 run that builds its vision tower; both are 2 bytes on device, so stick alignment is unaffected).
- **Tensor parallelism**: TP≥1 supported; `all_reduce` is provided natively by `libspyre_comms` (no longer overridden in `SpyreCommunicator`). **DP>1 is rejected** in `TorchSpyrePlatform.check_and_update_config` — the spyre-comms global rank space isn't validated for DP×TP. **bfloat16 + TP>1 is rejected there too**: torch-spyre's `all_reduce` is fp16-only on both paths (eager `SpyreCCLBackend` refuses bf16; the compiled `spyre.allreduce_plan` lowering has no bf16 `add`), so a bf16 model runs at TP1 only. A gemma-4 vision repo is the only bf16 model here, and `hf_overrides=GEMMA4_TEXT_BACKBONE_OVERRIDE` (`models/gemma4.py`) loads its decoder alone in fp16, restoring TP and quantization.
